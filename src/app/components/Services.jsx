...
const Services = () => {
  ...
  return (
    <section id="services" className='text-white p-4'>
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold">Our Services</h1>
        <p className="text-xl mt-4">Explore the Services we offer</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map(item => (
          <div key={item.id} layoutId={item.id} onClick={() => setSelectedId(item.id)}
          ...
          >
            ...
            {items.filter(item => item.id === selectedId).map((item) => (
              <React.Fragment key={item.id}>
                ...
                <h2 className="text-xl font-bold">{item.title}</h2>
                <p>{item.description}</p>
                <button className="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={() => setSelectedId(null)}>Close</button>
              </React.Fragment>
            ))}
          </div>
        )}
